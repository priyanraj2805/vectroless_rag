...
def process_pdf_background(pdf_path: str, groq_key: str, openrouter_key: str, redis_url: str):
    db = Database(settings.database_path)
    try:
        pipeline = IngestionPipeline(db, groq_key=groq_key, openrouter_key=openrouter_key, redis_url=redis_url)
...
async def upload_pdfs(files: List[UploadFile] = File(...), background_tasks: BackgroundTasks = None):
    ...
    for file in files:
        ...
        executor.submit(process_pdf_background, save_path, settings.groq_api_key, settings.openrouter_api_key, settings.redis_url)
